...
- **Decision (new)**: Promote the two hand-deployed Beast scripts (`/opt/amplified/apds/{harvest,label}/`) into version control at `02_build/apds/`. The harvester is byte-faithful with Beast; the labeller adds two minimal hardening edits over the Beast copy (escape PUDDING label values into the Cypher query; cast `confidence` to `float`) per Devin Review on PR #52. Beast copy is now one revision behind — sync deferred to the next session that re-runs the labeller on Beast (must `scp` the hardened copy first; otherwise re-running re-introduces the unescaped values). Add the matching infrastructure rows to `02_build/INFRASTRUCTURE.md` v3 (Graphiti container state change, APDS Stage-1 script entry).
